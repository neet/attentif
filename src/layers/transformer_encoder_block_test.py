...
def _ones_mask(batch_size, seq_len, dtype, device):
    """Create all-ones mask (all positions valid)."""
    return torch.ones(batch_size, seq_len, seq_len, dtype=dtype, device=device)

def _ban_last_k_columns(batch_size, seq_len, k, dtype, device):
    """Create mask that bans (sets to 0) the last k columns (key positions)."""
    mask = torch.ones(batch_size, seq_len, seq_len, dtype=dtype, device=device)
    mask[..., -k:] = 0.0  # key側の末尾k列を全面禁止 (0=masked, 1=valid)
    return mask
...
def test_forward_shape_dtype_device_cpu():
    ...
    mask = _ones_mask(batch_size=3, seq_len=7, dtype=x.dtype, device=x.device)
...
def test_attention_mask_actually_masks_keys():
    ...
    ones = _ones_mask(1, 6, x.dtype, x.device)
    ...
    # onesマスク（全部valid）：影響が出る（≠）
    y_ones_0 = m(x, attention_mask=ones).detach()
    y_ones_1 = m(x2, attention_mask=ones).detach()
    assert not torch.allclose(y_ones_0, y_ones_1, atol=1e-5, rtol=1e-5)
...
def test_eval_disables_dropout_deterministic():
    ...
    mask = _ones_mask(batch_size=2, seq_len=5, dtype=x.dtype, device=x.device)
...
def test_none_mask_equals_ones_mask():
    ...
    m, x = _make_block_and_inputs(batch_size=2, seq_len=5, hidden_size=16, num_attention_heads=4, device="cpu", dtype=torch.float32)
    ones = _ones_mask(batch_size=2, seq_len=5, dtype=x.dtype, device=x.device)
    ...
    torch.testing.assert_close(y_none, y_ones, atol=1e-6, rtol=1e-6)
